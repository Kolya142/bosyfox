...
modal.style.display="";
input.focus();
function closePrompt(result) {modal.style.display="none";
resolve(result);
...
input.onkeydown=(e)=>{if(e.key==="Enter")closePrompt(input.value);
...
curr_poses=[];
}}else if (((key)) === "I") {{prompt("Insert coord(example: \"500;200\"): ").then((s)=>{let splt  = s.split(";");
...
}}else if (((key)) === "G") {{prompt("Grid size(0 - disables)").then((s)=>{grid=Number(s);
redraw();
show();
updateFields();
});
}}else if (((key)) === "S") {{prompt("Enter color:").then((res)=>{curr_color=res;
redraw();
show();
updateFields();
});
